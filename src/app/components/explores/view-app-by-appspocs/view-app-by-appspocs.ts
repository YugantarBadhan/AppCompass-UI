...
@Component({
  selector: 'app-view-app-by-appspocs',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './view-app-by-appspocs.html',
  styleUrl: './view-app-by-appspocs.css',
})
export class ViewAppByAppspocs implements OnInit, OnDestroy {
  ...
  private loadApplicationsByTeam(teamName: string): void {
    ...
    this.applicationService.getAllAppBySpocs(teamName.trim()).subscribe({
      next: (applications) => {
        this.applications = applications || [];
        this.totalItems = this.applications.length;
        this.totalPages = Math.ceil(this.totalItems / this.itemsPerPage);
